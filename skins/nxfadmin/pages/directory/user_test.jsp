...
void quota_reset(UserTestDao dao){
	if(dao.quota_reset(param_str("name"))){
		succ_list.add("User Time Quota successfully reset");
	}
}
...
void bandwidth_reset(UserTestDao dao){
	if(dao.bandwidth_reset(param_str("name"))){
		succ_list.add("User Bandwidth Quota successfully reset.");
	}
}
%>
<%
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>NxFilter v<%= GlobalDao.get_nx_version()%> | Test User</title>
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
                ...
                <section class="content">
                    ...
                    <!-- start form -->
                    <form role="form" id="user_test_form" name="user_test_form" action="<%= get_page_name()%>" method="post">
                    ...
                    <input type="hidden" name="name" value="<%= data.name%>">
                            ...
                            <div class="box box-navy">
                                <div class="box-header">
                                    ...
                                    <h3 class="box-title">Test User Settings</h3>
                                </div><!-- /.box-header -->
                                <div class="box-body">
                                        ...
                                        <div class="form-group">
                                            <div class="input-group col-xs-3">
                                                ...
                                                <input type="text" class="form-control" id="group_name" name="group_name" value="<%= data.group_name%>" disabled>
                                            </div>
                                        </div>
                                        
                                        <div class="form-group">
                                            <div class="input-group col-xs-3">
                                                <label class="control-label" for="policy_name">User Policy</label>
                                                <input type="text" class="form-control" id="policy_name" name="policy_name" value="<%= data.policy_name%>" disabled>
                                            </div>
                                        </div>
                                        
                                        <div class="form-group">
                                            <div class="input-group col-xs-3">
                                                <label class="control-label" for="applied_policy_name">Applied Policy</label>
                                                <input type="text" class="form-control" id="applied_policy_name" name="applied_policy_name" value="<%= data.applied_policy_name%>" disabled>
                                            </div>
                                        </div>
                                     
                                        <div class="form-group">
                                            <div class="input-group col-xs-3">
                                                <label class="control-label" for="is_free_time">Free Time</label>
                                                <input type="text" class="form-control" id="is_free_time" name="is_free_time" value="<%= data.is_free_time ? "Yes" : "No"%>" disabled>
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <label class="control-label" for="quota_limit">Time Quota</label>
                                            <div class="input-group col-xs-3">
                                                <input type="text" class="form-control" id="quota_limit" name="quota_limit" value="<%= data.quota_used%> / <%= data.quota_limit%>" disabled >
                                                <span class="input-group-addon">minutes</span>
                                                <span class="input-group-btn">
                                                    <button id="resetQ" class="btn btn-primary" type="button">Reset</button>
                                                </span>
                                            </div>
                                        </div>
                                        
                                        <div class="form-group">
                                            <label class="control-label" for="bandwidth_limit">Bandwidth Quota</label>
                                            <div class="input-group col-xs-3">
                                                <input type="text" class="form-control" id="bandwidth_limit" name="bandwidth_limit" value="<%= data.bandwidth_used%> / <%= data.bandwidth_limit%>" disabled>
                                                <span class="input-group-addon">MB</span>
                                                <span class="input-group-btn">
                                                    <button id="resetB" class="btn btn-primary" type="button">Reset</button>
                                                </span>
                                            </div>
                                        </div>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function () {
                ...
                $('#resetQ').click(function() {
                    document.forms["user_test_form"].action_flag.value = "quota_reset";
                    document.getElementById("user_test_form").submit();        
                });

                $('#resetB').click(function() {
                    document.forms["user_test_form"].action_flag.value = "bandwidth_reset";
                    document.getElementById("user_test_form").submit();        
                });
                
                $('#user_test_form').submit(function(e) {
                    e.preventDefault();
                });
