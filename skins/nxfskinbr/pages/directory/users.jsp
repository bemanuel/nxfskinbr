...
void insert(UserDao dao){
	...
	if (pv.is_dup_user(data.name)) {
		err_list.add("Username already exists!");
		return;
	}

	if(dao.insert(data)){
		succ_list.add("User added successfully!");
	}
}
...
void delete(UserDao dao){
	if(demo_flag){
		err_list.add("Removing users not allowed on demo site!");
		return;
	}

	if(dao.delete(param_int("id"))){
		succ_list.add("User successfully removed.");
	}
}
%>
<%
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>NxFilter v<%= GlobalDao.get_nx_version()%> | Users</title>
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="box box-navy">
                                ...
                                <div class="box-body">
                                    <!-- start form -->
                                    <form role="form" id="addUser" action="<%= get_page_name()%>" method="post">
                                ...
                                <div class="box-footer">
                                    <button id="add" type="submit" form="addUser" class="btn btn-info">Add User</button>
                                </div>
                            </div><!-- /.box -->
                        </div><!-- /.col -->
                    </div><!-- /.row -->
                                ...
                                <div class="box-body">
                                    ...
                                    <table id="table" data-pagination="true" data-search="true" data-classes="table table-hover table-condensed" data-row-style="rowStyle">
                                        ...
                                        <tr>
                                            ...
                                            <th data-field="ip_line" data-sortable="true">IP</th>
                                            <th data-field="group_line" data-sortable="true">Group</th>
                                    ...
                                    <div class="callout callout-info">
                                        <h4>*&nbsp;I = IP,&nbsp;&nbsp;P = Password,&nbsp;&nbsp;L = LDAP,&nbsp;&nbsp;<span class="text-red">E = Expired</span></h4>
                                    </div>
...
for(int i = 0; i < data_list.size(); i++){
    ...
    out.println("{\"id\": " + data.id + ",\"name\": \"" + data.name + "\",\"type\": \"" + data.get_type() + "\",\"policy\": \"" + data.policy_name + "\",\"ip_line\": \"" + ip_line + "\",\"group_line\": \"" + group_line + "\",\"description\": \"" + data.description + "\"},");
...
window.operateEvents = {
    'click .test': function (e, value, row, index) {
        ...
        goForm(row.id, row.name, "", "user_test.jsp");
    },
    'click .edit': function (e, value, row, index) {
        //alert('Clicked edit icon, row: ' + JSON.stringify(row));
        goForm(row.id, row.name, "", "user_edit.jsp");
    },
    'click .remove': function (e, value, row, index) {
        $('#myModal').data('id', row.id);
        $('#myModal').data('name', row.name);
        $('#myModal').modal('show');
    }
};             
        </script>
