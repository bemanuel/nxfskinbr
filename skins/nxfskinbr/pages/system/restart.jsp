...
String os_type = param_str("optOS");
String isWin = "";
String isLin = " checked";
if(os_type.equals("win")){
    isWin = " checked";
} else if(os_type.equals("lin")){
    isLin = " checked";
}

  
/* 
if(action_flag.equals("restart")){
  String[] cmd = new String[3];
  if(os_type.equals("win")){
    cmd[0] = "cmd";
    cmd[1] = "/c";
    cmd[2] = "net start NxFilter";                  // Command string specific to running NxFilter as a windows service.  Edit as necessary!!!
  } else if(os_type.equals("lin")){
    cmd[0] = "/sbin/initctl restart nxd";           // Command string specific to using my upstart script for NxFilter.  Edit as necessary for path and names!!!
  } else {
    err_list.add("Unknown OS type selected!");
  }
  
  try
  {
    //out.println(os_type + " Restart with " + cmd);
  ...
  succ_list.add("NxFilter process restarting...");
}
*/

if(action_flag.equals("restart")){
  if(os_type.equals("lin")){
      try
      {
          Runtime r = Runtime.getRuntime();
          String cmd = "/sbin/initctl restart nxd";       // Command string specific to using my upstart script for NxFilter.  Edit as necessary for path and names!!!
          Process p = r.exec(cmd);
          p.waitFor();
          p.destroy();
      } catch(Exception e) {
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>NxFilter v<%= GlobalDao.get_nx_version()%> | RESTART</title>
                        ...
                        <div class="col-xs-12">
                            <div class="box box-danger">
                                <div class="box-header">
                                    ...
                                    <h3 class="box-title">Restart NxFilter</h3>
                                </div><!-- /.box-header -->
                                <div class="box-body">


                               <form role="form" action="<%= get_page_name()%>" method="post">
                                    <input type='hidden' name='action_flag' value='restart'>
                                    <div class="box-body">
                                        <div class="form-group">
                                            <div class="alert alert-info alert-dismissable">
                                                <i class="fa fa-info"></i>
                                                <b>Restart NxFilter only when necessary.</b><br />Current <%= os.getName()%> System Load Average: <%= os.getSystemLoadAverage()%>
                                            </div>
                                            <blockquote>
                                            Restarting NxFilter will stop all client access to NxFilter services temporarily and may cause this web interface to become unresponsive for a short time.
                                            </blockquote>
                                        </div>
                                        <div class="form-group">
                                            <label>
                                                <input type="radio" name="optOS" class="flat-red" value="lin" <%= isLin %> />
                                                NxFilter on linux controlled using Upstart
                                            </label>
<!--
                                            <br />
                                            <label>
                                                <input type="radio" name="optOS" class="flat-red" value="win" <%= isWin %> disabled/>
                                                NxFilter running on Windows as a service
                                            </label>
-->
                                        </div>
                                    </div><!-- /.box-body -->
                                    ...
                                    <div class="box-footer">
                                        <button type="submit" class="btn btn-info btn-lg"><strong>Restart NxFilter</strong></button>
                                    </div>
                                </form>
